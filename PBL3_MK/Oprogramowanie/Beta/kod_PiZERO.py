...
#kod na Raspberry Pi Zero
#Stacja Pi Zero zbierajaca dane  z BME280 i DS18B20 i wysylajaca ramkami 32B do Pi4b
# Pi zero wykonuje pomiary temperatury z DS18B20 oraz temperatury i wilgotnosci z BME280
# co okreslony czas (w tym wypadku 30s), co 5 minut liczy srednia i wysyla dane poprzez
# modul lora sx1262, do ramki dodawana jest mniejsza z liczby wykonanych pomiarow - co pozwala
# zauwazyc anomalie, jest rowniez podawany czas 
# ramka bedzie w przyszlosci zmieniana aby uwzglednic pomiary predkosci wiatru
# i parametry panujace wewnatrz stacji (np. czy wilgotnosc w srodku puszki instalacyjnej nie jest niebezpieczna)
...
def budowanie_ramki(id_stacji, temp_ds, temp_bme, wilg_bme, liczba_probek):
    czas = time.strftime("%H%M%S")
    ramka = f"{id_stacji}|{format_temp(temp_ds)}|{format_temp(temp_bme)}|{format_wilg(wilg_bme)}|{liczba_probek:02d}|{czas}"
    return ramka.encode('utf-8')[:32].ljust(32)

def main():
    print("Stacja przymrozkowa ZERO")
    ...
    try:
        while True:
            ...
            if time.time() - ostatnie_wyslanie >= INTERWAL_WYSYLANIA:
                ...
                ramka = budowanie_ramki(ID_STACJI, sr_ds, sr_bme_t, sr_bme_h, n)
                czas = time.strftime("%H:%M:%S")
